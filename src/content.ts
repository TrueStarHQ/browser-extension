...
class TrueStarAnalyzer {
  ...
  private async callOpenAI(reviews: ReviewData[]): Promise<any> {
    const prompt = `Analyze these Amazon product reviews for fake review patterns. Look for:
...
Reviews to analyze:
${reviews.map((r, i) => `Review ${i + 1}: ${r.rating}/5 stars - "${r.text}" (Verified: ${r.verified})`).join('\n')}
